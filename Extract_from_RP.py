...
def First_middle_last_avg(DICT, var, win_loc):
  ...
  '''
  Outcome_data=pd.read_csv(group_outcome_file)
  ...
  DICT={'label':label,var:vars,'outcome':out,'window':window,'group':GID}
  data=pd.DataFrame.from_dict(DICT)
  #sns.displot(data=data, x=var, kde=True,hue=['label','outcome'],kind='hist',norm_hist=True)
  return data
  
def First_middle_last_sliding_windows(DICT, var, save_dir):
  '''
  ...
  print(a)
  full_data.to_csv(save_dir+'/'+var+'_diff_windows.csv')
...
def First_middle_last_sliding_windows_all_vars(DICT):
  '''
  ...
  for var in tqdm(vars):
    df=First_middle_last_sliding_windows(DICT_GD, var)
    dfs.append(df)
